...
class Controller {
    ...
    handleMQTTMessageDevice(topic, message, withPrefix) {
        ...
        Object.keys(json).forEach((key) => {
            ...
            const message = converter.convert(json[key], json, mappedModel);
            ...
            if (converter.type === 'functional') {
                this.zigbee.functional(deviceID, message.cid, message.cmd, message.zclData, ep, callback);
            } else if (converter.type === 'write') {
                this.zigbee.write(deviceID, message.cid, message.attrid, message.data, ep, callback);
            }
        ...
        published.forEach((p) => {
            ...
            if (p.message.hasOwnProperty('zclData') && p.message.zclData.hasOwnProperty('transtime')) {
