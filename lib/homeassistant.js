...
const configurations = {
    ...
    'climate': {
        ...
        discovery_payload: {
            min_temp: 20,
	    max_temp: 45,
	    modes: ['heat','off','auto','cool'], 
            platform: 'mqtt',
            //command_topic: 'command_topic',
	    current_temperature_topic: 'value_topic',
            value_template: '{{ value_json.temperature }}', //'> {% if value_json != undefined %} {% set trash = value_json.temperature %} {{ trash }} {% else %} -1 {% endif %}',
	    temperature_command_topic: 'command_topic', 
	    temperature_state_topic: 'value_topic',
	    temperature_state_template: '{{ value_json.temperatureSet }}',// '> {% if value_json != undefined %} {% set trash = value_json.temperatureSet %} {{ trash }} {% else %} -1 {% endif %}',// '{{ value_json["temperatureSet"] }}',
	    mode_command_topic: 'command_topic',
	    mode_state_topic: 'value_topic',
            mode_state_template: '{{ value_json.state }}',//'> {% if value_json != undefined %} {% set trash = value_json.state %} {{ trash }} {% else %} -1 {% endif %}',//'{{ value_json["state"] }}',
	    json_attributes: ['relaystate', 'linkquality'],
        },
    },
    'sensor_lqi': {
        type: 'sensor',
	    ...
	    platform: 'mqtt',
            ...
            value_template: '{{ value_json.linkquality*97//255-87 }}',
...
const mapping = {
    ...
    'ZBESTTHERM01':[configurations.climate, configurations.sensor_lqi],
};
...
function discover(deviceID, model, mqtt, force) {
	...
	//console.log(deviceID +'=' + friendlyName );
	...
	for (key in payload){
		if (/command_topic/.test(key))
		{
			//if (payload[key]==='command_topic'){
				payload[key] = `${settings.get().mqtt.base_topic}/${friendlyName}/set`;	
		
 /*           			payload[key] += 'set';
			}else if (payload[key]==='value_topic') 
				payload[key] = `${settings.get().mqtt.base_topic}/${friendlyName}`;*/
		
		}
		if (/state_topic/.test(key))
		{
			payload[key] =`${settings.get().mqtt.base_topic}/${friendlyName}`;
		}
		if (/current_temperature_topic/.test(key))
		{
			payload[key] =`${settings.get().mqtt.base_topic}/${friendlyName}`;
		}
