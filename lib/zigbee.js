...
class Zigbee {
    ...
    functional(deviceID, cid, cmd, zclData, ep, callback) {
        ...
        logger.info(
            `Zigbee publish to '${deviceID}', ${cid} - ${cmd} - ${JSON.stringify(zclData)} - ${ep} - functional`
        );

        device.functional(cid, cmd, zclData, callback);
    }

    write(deviceID, cid, attrid, data, ep, callback) {
        const device = this._findDevice(deviceID, ep);
        if (!device) {
            logger.error(`Zigbee cannot publish message to device because '${deviceID}' not known by zigbee-shepherd`);
            return;
        ...
        logger.info(`Zigbee publish to '${deviceID}', ${cid} - ${attrid} - ${data} - ${ep} - write`);
        device.write(cid, attrid, data, callback);
    }
